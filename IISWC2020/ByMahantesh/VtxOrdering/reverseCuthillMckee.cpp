        ...
        while ( QTail > 0 ) {
            ...
            for (long Qi=0; Qi<QTail; Qi++) {
                ...
                for(long k = adj1; k < adj2; k++) {
                    ...
                    //Has this neighbor been visited?
                    if ( visited[x] == 0 ) {
                        //Not visited: add it to the Queue and mark it as visited
                        Qtmp[QtmpTail] = x;
                        visited[x] = 1;
                        QtmpTail++;
                        indexWeight newNode( x, degree[x] ); //Add for sorting
            ...
            for (long iQtmpTail=0; iQtmpTail < QtmpTail; iQtmpTail++ ) //O(n)
            {
                ...
                howManyAdded++;
                Qtmp[iQtmpTail] = sortedWeightList[iQtmpTail].getIndex(); //Add it to the queue as well
            }
